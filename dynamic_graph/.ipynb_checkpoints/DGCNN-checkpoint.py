...
def knn(x, k):
    ...
    del pairwise_distance, xx, inner
    return idx
...
class DGCNN_semseg(nn.Module):
    def __init__(self, k, emd_dims, dropout):
        super(DGCNN_semseg, self).__init__()
        self.k = k
        self.emb_dims = emd_dims
    ...
    def forward(self, x):
        ...
        x = get_graph_feature(x, k=self.k, dim4=True)   # (batch_size, 3, num_points) -> (batch_size, 3*2, num_points, k)
        ...
        x = self.dp1(x)
        x = self.conv9(x)                       # (batch_size, 256, num_points) -> (batch_size, 1, num_points)
