...
def DICE_from_confusions(confusions):
    """
    Computes DICE from confusion matrices.
    :param confusions: ([..., n_c, n_c] np.int32). Can be any dimension, the confusion matrices should be described by
    the last axes. n_c = number of classes
    :param ignore_unclassified: (bool). True if the the first class should be ignored in the results
    :return: ([..., n_c] np.float32) IoU score
    ...
    # Compute IoU
    DICE = 2*TP / (TP_plus_FP + TP_plus_FN + 1e-6)

    # Compute mIoU with only the actual classes
    mask = TP_plus_FN < 1e-3
    counts = np.sum(1 - mask, axis=-1, keepdims=True)
    mIoU = np.sum(DICE, axis=-1, keepdims=True) / (counts + 1e-6)
...
def shapenetpart_metrics(num_classes, num_parts, objects, preds, targets, masks, is_conf = False, is_1_class_iou = True):
    """
    Args:
        num_classes:
        num_parts:
    ...
    for l in range(num_classes):
        ...
        obj_DICE = DICE_from_confusions(obj_confs)
        obj_dices += [obj_DICE[:,-1]]
        if is_1_class_iou:
            obj_mIoUs += [obj_IoUs[:,-1]]
        else:
    ...
    objs_average_dice = [np.mean(mIoUs) for mIoUs in obj_dices]
    instance_average_dice = np.mean(np.hstack(obj_dices))
    class_average_dice = np.mean(objs_average_dice)
...
#     print('-----|------|--------------------------------------------------------------------------------')
    ...
    #print(s + '\n')
    if is_conf:
        return acc, objs_average, class_average, instance_average, np.sum(Confs, axis = 0), class_average_dice
    else:
        return acc, objs_average, class_average, instance_average,class_average_dice
